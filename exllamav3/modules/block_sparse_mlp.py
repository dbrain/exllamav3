    ...
    def forward(
        ...
        # Torch/C++/fused path
            ...
            if self.num_local_experts is None or self.num_local_experts > 0:
                ...
                # Count how many assignments per expert. Consumed on the DEVICE by
                # run_fused below, and on the host only in the overflow branch, so
                # counting must not itself sync -- see _expert_counts
