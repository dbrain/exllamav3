...
def _grouped_dequant_gemv_kernel(
    ...
    # --- BEGIN generated by _gen.py from exl3_triton.py (sha256[:16] 9beaeda447f0f386) ---
...
def fuse_gate_up() -> bool:
    ...
    That "unexplained 26 us" verdict stood on two interleaved reps at depth
    4096 (9.4042 vs 8.7108 t/s, a 7.66% spread on one arm) and said "default
    OFF, do not ship on this evidence". Round 2 settled it with an interleaved
    in-process A/B, n=15 per arm, and the answer is SHIP -- but only for one
    shape:

        seqlen 1   ON 109.315 [ 98.890-110.855]   OFF 118.252 [115.927-119.563]
                   x1.0818 DISJOINT;  moe_fwd 34.608 -> 43.548 (+8.94 ms)
        M=7        ON 241.231 [237.183-247.634]   OFF 240.989 [236.659-244.952]
                   x0.9990 OVERLAP;  moe_fwd 116.607 -> 116.510 (+0.10 ms)

    So it is worth +8.2% in plain bsz-1 decode and is measurably NOTHING at the
    speculative verify shape, which is 84% of the shipped round. The merge only
    pays when the grid is small enough for 2E vs E programs to matter; at M=7
    the grid is already 7x larger and the flag disappears. The 26 us is still
    unexplained and still should not carry a theory.
    """
